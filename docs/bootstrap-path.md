...
We're ready now to bootstrap the final compiler and code generator.

Unlike the previous code generator, this one consumes an [intermediate representation (IR)](intermediate-representation.md). It's similar to assembly language; the main difference is instead of a limited number of registers, we have unlimited temporaries.

The code generator performs optimizations, register allocation, stack frame layout and more. We don't need to do any of that stuff in the compiler proper so it can focus entirely on parsing C.

#### [cci/2-full](../core/cci/2-full/)

This is the full C compiler. We aim to support all of C23 with many extensions.
...
Unlike previous stages, this one emits IR rather than assembly. It requires the final code generator to turn it into assembly. It is much easier to generate IR, and we get far better optimizations as a result.
...
We also install all of the Onramp binaries (`cpp.oe`, `cci.oe`, `cg.oe`, `ar.oe`, `ld.oe`), the libc archive (`libc.oa`) and the libc headers. The driver will access all of these as needed to compile programs.
